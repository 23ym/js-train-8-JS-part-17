...
function getUsersFromDatabase(array) {
  const userArray = Array.from(array, (user) => ({ // Використовуємо метод `Array.from` для створення масиву користувачів зі списку, елементи якого це об'єкти які міститять
    id: user.id, 
...
function filterByCondition(arr, condition) {
  const filteredArray = arr.filter(condition); // Використовуємо метод `filter` для фільтрації масиву
  return filteredArray; // Повертаємо відфільтрований масив
}
...
function checkArray(obj) {
  if(obj === null || obj === undefined) { // Перевіряємо, чи є об'єкт не null і не undefined
    console.error("Помилка: Вхідний об'єкт є null або undefined."); // Якщо об'єкт є null або undefined, виводимо повідомлення про помилку Помилка: Вхідний об'єкт є null або undefined.
  return false; //Повертаємо false
  } 

  const isArray = Array.isArray(obj); // Використовуємо метод `Array.isArray` для перевірки, чи є об'єкт масивом

  return isArray; // Повертаємо результат перевірки
...
function createArray(...elements) {
  if (elements.length === 0) {
    console.error("Помилка: Не переданожодного елементу.");
    return [];
  }
  // Перевіряємо, чи кількість переданих елементів більше нуля
  const newArray = Array.of(...elements);// Використовуємо метод `Array.of` для створення нового масиву з переданих елементів
...
function getElementAtIndex(arr, index) {
  if (!Array.isArray(arr) || arr.length === 0) { // Перевіряємо, чи масив не є порожнім
  ...
  if (index >= 0 && index < arr.length) { // Перевіряємо, чи індекс знаходиться в межах довжини масиву
    const element = arr.at(index); // Отримуємо елемент з масиву за заданим індексом
...
function combineAndReverseArrays(arr1, arr2) {
  ...
  const combinedArray = arr1.concat(arr2); // Об'єднуємо два масиви за допомогою методу `concat`
  const reversedArray = combinedArray.reverse(); // Обертаємо отриманий об'єднаний масив за допомогою методу `reverse`
  return reversedArray;// Повертаємо об'єднаний та обернутий масив
}
...
function findElementIndexes(arr, element) {
  ...
  if (firstIndex !== -1) { 
    indexes.push(firstIndex); // Додаємо перший знайдений індекс, якщо елемент знайдено
  }
...
function copyAndSwapElements(arr, target, start, end) {
  if (target < 0 || target >= arr.length || start < 0 || start >= arr.length || end < 0 || end > arr.length || start >= end) {
    return arr; // Перевіряємо, чи індекси належать межам масиву якщо ні виводимо рядок Неприпустимі індекси
  }
...
function customEvery(arr, condition) {
  if (!Array.isArray(arr) || typeof condition !== 'function') { // Перевірка вхідних параметрів
    return false; // якщо arr не масив повертаємо false
  }
  // якщо condition не function повертаємо false
   
  const result = arr.every(condition); // Використання методу `every` для перевірки умови для кожного елементу масиву
...
function customFill(arr, value, start = 0, end = arr.length) {
  if (!Array.isArray(arr) || start < 0 || end > arr.length || start > end) {// Перевірка вхідних параметрів
    return false; // якщо arr не масив повертаємо false
  }
  arr.fill(value, start, end); // якщо start та end не входять в діапазон від 0 до довжини масиву arr повертаємо false
...
function customUnshift(arr, ...elements) {
  ...
  for (let i = elements.length - 1; i >= 0; i--) { // Додаємо елементи на початок масиву за допомогою методу `unshift` та циклу for, початкове значення лічильника на 1 менше ніж довжина масива,
...
function customAt(arr, index) {
  ...
  if (typeof element === 'string') { 
    ...
    console.log('Елемент є обєктом'); // перевіремо якого типу елемент ми отримали,якщо рядок виведом в консоль -Елемент є рядком,числом - Елемент є числом, об`єктом -Елемент є обєктом
}
  return element;
}
...
function customIncludes(arr, element) {
  ...
  const result = arr.includes(element); // Використовуємо метод includes для перевірки наявності елемента в масиві
  const count = arr.filter((item) => item === element).length; 
  console.log(count); // За допомогою методу filter перевіряємо скільки разів в масиві зустрічається елемент та виводимо число в консоль
